...
int HotPlug::hotplug_callback(struct libusb_context* ctx, struct libusb_device* device, libusb_hotplug_event event,
                              void* user_data) {
...
void* HotPlug::usb_listener_method(void* data) {
    ...
    if (!libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
        ...
        int result_Register = libusb_hotplug_register_callback(
            context, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED | LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT, 1, vendor_id, product_id,
            device_class, hotplug_callback, (void*)&hogPlug, &handle);
